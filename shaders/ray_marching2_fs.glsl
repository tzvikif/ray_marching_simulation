...
vec3 phongContribForLight(vec3 k_d, vec3 k_s, float alpha, vec3 p, vec3 eye,
    ...
    vec3 L = normalize(lightPos - p); //direction from p to light
    vec3 V = normalize(eye - p); //direction from p to eye
    vec3 R = normalize(reflect(-L, N));
    
    float dotLN = dot(L, N);	//dot(lightDir,normal)
    float dotRV = dot(R, V);	//dot(reflection,viewDir)
...
vec3 phongIllumination(vec3 k_a, vec3 k_d, vec3 k_s, float alpha, vec3 p, vec3 eye) {
    ...
    color += phongContribForLight(k_d, k_s, alpha, p, eye,
                                  light2Pos,
                                  light2Intensity);    
    vec3 light3Intensity = vec3(0.8, 0.4, 0.4);
...
void main()
{
    ...
    //vec2 fragCoord = v_uv-0.5;
    //vec2 fragCoord = v_uv;
    //vec2 q = fragCoord;
    //q.x = fragCoord.x  * (iResolution.x/iResolution.y);
